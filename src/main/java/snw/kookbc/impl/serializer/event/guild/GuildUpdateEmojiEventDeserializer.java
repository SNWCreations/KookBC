...
public class GuildUpdateEmojiEventDeserializer extends NormalEventDeserializer<GuildUpdateEmojiEvent> {
    ...
    protected GuildUpdateEmojiEvent deserialize(JsonObject object, Type type, JsonDeserializationContext ctx,
        ...
        final CustomEmoji customEmoji = client.getStorage().getEmoji(body.get("id").getAsString(), body);
     ...
     */
    @Override
    protected GuildUpdateEmojiEvent deserializeFromNode(JsonNode node, long timeStamp, JsonNode body) {
    ...
    protected boolean useJacksonDeserialization() {
        // CustomEmojiImpl已支持Jackson update，可以启用
        return true;
    }
